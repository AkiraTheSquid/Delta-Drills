...
def get_runtime_dir() -> Path:
    runtime_dir = os.environ.get("DELTA_MATHPIX_RUNTIME_DIR", "").strip()
    if not runtime_dir:
        runtime_dir = os.environ.get("DELTA_MATHPIX_CODE_DIR", "").strip()
    if not runtime_dir:
...
def _append_history_line(cmd_line: str):
    try:
        history_path = get_runtime_dir() / "mathpix_commands_history"
...
if __name__ == "__main__":
    def _set_completion_state(value: str):
        try:
            state_path = get_runtime_dir() / "completion_state.txt"
