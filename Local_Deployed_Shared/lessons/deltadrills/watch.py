"""watch.py — health checks for lessons/deltadrills (the Delta Drills course)
...
is proven by scripts/validate_math.py. The deep checks live in that script,
which lessons/watch.py already runs on the whole tree; this watch is the
folder-local half — the shape rules that make a page reach it. Mirrors
../mathematics/watch.py, the other kind: math folder.
...
def check_public_api():
    ...
    for slug in sorted(pages):
        with open(os.path.join(_DIR, slug + ".md"), encoding="utf-8") as fh:
            head = fh.read(2000)
        assert "\nkind: math\n" in head, f"{slug}.md is in lessons/deltadrills but is not `kind: math`"
        data = math_bank.read_problem_file(Path(_DIR) / (slug + ".problems.json"))
        assert data["kc"].startswith("deltadrills."), (
            f"{slug}.problems.json kc {data['kc']!r} is not a deltadrills.* id"
        )
