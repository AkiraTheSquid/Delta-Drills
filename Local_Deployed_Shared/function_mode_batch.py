...
from delta_paths import REPO_DIR, SHARED_DIR, THIS_DIR_ONLY, get_backend_python, get_chatgpt_code_dir, get_chatgpt_runtime_dir


CODE_DIR = SHARED_DIR
CHATGPT_CODE_DIR = get_chatgpt_code_dir()
RUNTIME_DIR = get_chatgpt_runtime_dir()
...
async def main() -> None:
    ...
    api_key = load_api_key(str(CHATGPT_CODE_DIR))
    if not api_key:
        raise RuntimeError("No OpenAI API key configured in chatgpt/.")

    client = AsyncOpenAI(api_key=api_key)
    model = os.environ.get("OPENAI_MODEL", get_configured_model(str(CHATGPT_CODE_DIR))) or "gpt-4o-mini"
    lock = asyncio.Lock()
