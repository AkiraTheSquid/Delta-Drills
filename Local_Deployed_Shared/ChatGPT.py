...
def get_runtime_dir(code_dir: str) -> str:
    runtime_dir = os.environ.get("DELTA_CHATGPT_RUNTIME_DIR", "").strip()
    if not runtime_dir:
        return code_dir
...
def get_code_dir(script_dir: str) -> str:
    code_dir = os.environ.get("DELTA_CHATGPT_CODE_DIR", "").strip()
    if not code_dir:
        return script_dir
...
def main() -> None:
    ...
    code_dir = get_code_dir(script_dir)
    runtime_dir = get_runtime_dir(code_dir)
    prompt_path = os.path.join(runtime_dir, "prompt.txt")
    output_path = os.path.join(runtime_dir, "output.txt")
    meta_path = os.path.join(runtime_dir, "output.meta.json")
    lock_path = os.path.join(runtime_dir, "output.lock")
    completion_state_path = os.path.join(runtime_dir, "completion_state.txt")
        ...
        if simple_mode:
            ...
            try:
                atomic_write_text(meta_path, json.dumps({
                    "run_id": run_id,
                    "model": get_configured_model(code_dir),
    ...
    if not api_key:
        ...
        api_key = prompt_and_store_api_key(code_dir)
        if not api_key:
