...
def get_runtime_dir(base_dir: str) -> str:
    runtime_dir = os.environ.get("DELTA_CHATGPT_RUNTIME_DIR", "").strip()
    return runtime_dir or base_dir
...
async def main_async():
    ...
    code_dir = str(get_chatgpt_code_dir())
    runtime_dir = str(get_chatgpt_runtime_dir())
    prompts_dir = os.path.join(runtime_dir, "prompts")
    outputs_dir = os.path.join(runtime_dir, "outputs")
    usage_file = os.path.join(runtime_dir, "usage.md")
    completion_state_file = os.path.join(runtime_dir, "completion_state.txt")
    ...
    try:
        ...
        settings = parse_settings(code_dir)
        ...
        used_prompt_nums = {num for num, _, _, _ in prompts_with_tokens}
        cleanup_output_dir(outputs_dir, used_prompt_nums)

        print(f"Found {len(prompts_with_tokens)} prompts to process.", file=sys.stderr)

        total_estimated_tokens = sum(tokens for _, _, tokens, _ in prompts_with_tokens)
