...
- 2026-09-01 (**the auditor runs the learner's actual surfaces**): `audit_question_bank.py` +2 checks. (1) `wrong_example_matches_correct` (BLOCKING) — a `wrong_examples` entry whose output equals the correct output for its anchor (its own `call` snippet when present, else the example run's `expected_output`) tells a learner with the RIGHT answer they hold the misconception. Fired on q713 (its `(True, True)` "no conversion" example is also the correct output for `(5, 'hi')`); fixed by anchoring the entry to `solve(0, '')`, where the misconception witnesses itself. q484's call-anchored entry passes. (2) `check_starter_harness` (`--confirm` only) — runs the starter AS HANDED through `run_function_tests`; passing every case is `starter_passes_all_cases` (BLOCKING), a partial pass is `starter_passes_some_cases` (informational — grading is all-or-nothing, `grading.py: correct = all(...)`). First bank-wide run: **all 469 answers pass their own cases (never verified before), 0 starters pass all, q550 passes 2/4** (its two None-expected cases match the placeholder). Both checks mutation-verified: a synthetic all-None-cases starter blocks, mutated q484 fires, a broken `call` snippet reports `wrong_example_call_error` instead of skipping. Motivation: docs/spec-graph-metadata-audit-layer.md §4.1.

- 2026-08-30: `retired_question_ids.json` added, and `export_questions_json.py` skips its ids. This is how a drill is retired for CURRICULUM reasons, and it is deliberately separate from `chatgpt/function_mode_deleted_ids.json`, which retires drills whose function-mode payload is broken — the two reasons have different lifetimes and a concept coming back should not resurrect a broken payload. 🔴 Never retire a drill by deleting its CSV row: ids are POSITIONAL in the CSV sources, so a deleted row renumbers every question below it and silently re-points every qmatrix tag and every stored `served_question_id`. The exclusion is applied after the id is assigned, exactly like the two lists that were already here. 216 ids retired in the ARENA content cut; bank 586 → 370.
- 2026-08-28 (**`wrong_examples`: authored near-miss output per question**): `export_questions_json.py`.
  - New override field `[{call, output, why}]`, carried on both the flat bank and `questions_structured.json`'s `exercise` block, rendered under the prompt by `practice/question-examples.js`. Override-only by design: there is no honest way to DERIVE a wrong answer, and the `why` is the part that teaches.
  - 🔴 **Author the OUTPUT by RUNNING a wrong implementation on one of the drill's own grader inputs, never by hand** — and on an input where right and wrong actually disagree. A misconception that passes every case is not a near miss, it is a second correct answer. Two of the 45 authored for `numpy.ndarray-model` had to be rewritten for exactly that reason.
  - 🔴 The override whitelist here and in `backend/app/questions.py` must MATCH; `watch.py check_invariants` fails the moment they diverge.
- 2026-08-18: The writer of that runtime layer moved off the server. A flagged question is now queued (`ai_feedback_queue.jsonl`, same directory) and repaired by `ops/question_repair/run_repairs.py` on Seth's machine through the local `claude` CLI; the file this folder reads, and everything below, is unchanged. One thing that gets easier: the runner already re-runs a rewritten `answer_code` against the question's `test_cases` before it is applied, so a repair that reaches the layer has passed the same harness `test_torch_grading.py` guards. One thing that does not: promoting production's repairs into the shipped bank is still a manual copy off the volume before export.
- 2026-08-13: `load_function_overrides()` gained a final layer that no script in this folder produces — `ai_feedback_overrides.jsonl`, written at runtime by the backend when a learner flags a question and a model repairs it (`backend/app/practice/feedback_ai_improver.py`). It is registered after `curated_overrides.jsonl` in both lists as usual, but breaks the folder's normal assumption in two ways. It is **not read from `CHATGPT_RUNTIME_DIR`**: `_feedback_ai_layer_path()` honours `DELTA_FEEDBACK_AI_DIR`, which production points at the Fly `/data` volume, because anything in the image is rebuilt by a deploy and a runtime write there would be lost. And it is **not in the repo**, so the third step of registering a layer (`git add -f`) does not apply — on a dev box the file is simply absent and the layer is empty. To fold production's repairs into the shipped bank, copy `ai_feedback_overrides.jsonl` down off the volume before exporting; `ai_feedback_revisions.jsonl` beside it holds the before/after for each one and is what to read before deciding whether a repair deserves promoting.
