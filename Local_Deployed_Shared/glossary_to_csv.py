...
def _run_chatgpt(prompt: str, model: str | None = None, label: str | None = None) -> str:
    ...
    meta_path = runtime_dir / "output.meta.json"

    prompt_path.write_text(prompt, encoding="utf-8")
    if label:
        (runtime_dir / f"{label}_prompt.txt").write_text(prompt, encoding="utf-8")
    ...
    if label:
        (runtime_dir / f"{label}_output.txt").write_text(answer, encoding="utf-8")
        if meta_path.exists():
            (runtime_dir / f"{label}_meta.json").write_text(meta_path.read_text(encoding="utf-8"), encoding="utf-8")
    return answer
...
def _run_splitter(pdf_path: Path, chapters_csv: Path, output_dir: Path) -> int:
    splitter = SCRIPT_DIR / "split_LADR4e_chapters.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mathpix-out", default=str(THIS_DIR_ONLY / "toc_md"), help="Output folder for Mathpix Markdown")
    parser.add_argument("--csv-path", default=str(THIS_DIR_ONLY / "toc.csv"), help="Output CSV path")
    ...
    parser.add_argument("--chapters-csv", default=str(THIS_DIR_ONLY / "toc_chapters.csv"), help="Output chapters CSV path")
    ...
    toc_json_text = _run_chatgpt(prompt, model=args.model, label="toc_detection")
    ...
    csv_text = _run_chatgpt(csv_prompt, model=args.model, label="toc_csv")
    ...
    if args.split:
        if args.chapters_dir:
            ...
            chapters_dir = THIS_DIR_ONLY / f"{pdf_path.stem}_chapters"
