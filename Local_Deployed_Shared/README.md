...
## Recent Changes
- 2026-09-24: `domain-move.js` — the app moved to https://deltadrills.com. Loaded FIRST in `index.html`'s `<head>`: on `delta-drills.vercel.app` it stops the page and hops to the same path on the .com, carrying localStorage in the `#dd-move=` fragment on a browser's FIRST hop only (`dd_domain_move_sent`); the .com writes keys it doesn't already have, then reloads clean. Sign-in token + API keys are never carried (fragments land in synced browser history). Transitional: retire by switching the vercel.app domain to a Vercel redirect and deleting the file + tag.
