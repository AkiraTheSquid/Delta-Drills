...
- `timer.js`: rigid practice session — the learner sets question count + strict answer/review time per question up front (`#practice-session-setup` panel), then `PracticeSession` enforces them: answer expiry force-submits (or force-advances when nothing is submittable, e.g. torch Colab routing), review expiry auto-rates the default felt-difficulty and force-advances. Hooks: `onQuestionRendered` (ui.js), `pauseForGrading`/`resumeAnswerPhase`/`beginReviewPhase` (events.js submit), `shouldFinishInsteadOfAdvance` (events.js `_loadNextPracticeQuestion` quota gate). Setup defaults persist in `localStorage.delta_drills_session_setup`. Styles in `../styles/practice/timer.css`.
...
## Recent Changes
- 2026-07-12 (rigid practice sessions): Replaced the optional timed-mode toggle with a mandatory session setup gate. The practice tab now opens on a setup panel (question count, answer time/question, review time/question); questions only appear after Start. Timers are strict — no pause control, answer expiry auto-submits, review expiry auto-rates "About right" + advances, session ends after the committed question count. `timer.js` rewritten (`PracticeSession` module); `dom.js`/`ui.js`/`events.js` re-hooked; old toggle markup removed from `index.html`. Sibling `practice.css` was also split into `../styles/practice/*.css` the same day.
