...
- 2026-07-31 (the Colab edition — a second Vercel project, same code): New `colab_mode.js`, loaded before `ui.js` (`index.html`, `ui.js?v=24`). `delta-drills-colab.vercel.app` serves the identical frontend and routes a drill to its lesson notebook; `delta-drills.vercel.app` is untouched. Deployed by `This-Directory-Only/scripts/deploy_delta_drills_colab.sh` off the `deploy-colab` branch, sharing the Fly backend, Supabase state and question bank — practice on either moves the same mastery record.
  - **The route was unreachable before this, and not because of a flag.** `torchNeedsColab` required `q.problem_notebook_path || q.solution_notebook_path`, and both are empty on all 499 bank rows; forcing `practiceMode` off `backend` would still have produced nothing. The join now runs `question.subtopic_key → lessons/colab_notebooks.json → <lesson>.ipynb`, the map `scripts/generate_colab_notebooks.py` writes beside the notebooks it publishes, so it cannot drift from what is on GitHub.
  - **387 of 499 questions resolve; the other 112 stay on the editor.** Autograd and CNN lessons have no generated notebook. Routing hides the editor, the aids and the submit bar, so a question sent to a notebook that does not exist is a dead end with no controls — `colabNotebookHrefFor` returning `""` is the guard, and `torchNeedsColab` keeps the pre-existing "somewhere to route TO" rule for exactly the same reason.
