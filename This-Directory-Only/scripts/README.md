...
- `deploy_delta_drills_colab.sh`: **DEPRECATED 2026-09-22** — published the Colab edition (`delta-drills-colab.vercel.app`, the same frontend under a second Vercel project). Disconnected from the main deploy and now exits 1 without deploying.
...
10. (Removed 2026-09-22) The Colab edition is deprecated and no longer republished.
...
## Recent Changes
- 2026-09-22: **Colab edition deprecated and disconnected** (Seth). Step 7 of `deploy_delta_drills.sh` no longer calls `deploy_delta_drills_colab.sh`, which now refuses to run; `watch.py` asserts the main deploy never calls it. The sync had failed on every deploy since the edition was abandoned on 09-13.
