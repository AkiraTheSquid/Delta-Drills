...
- `deploy_delta_drills_colab.sh`: publishes the Colab edition — the SAME frontend under a second Vercel project (`delta-drills-colab`), where `practice/colab_mode.js` reads `location.hostname` and routes a drill to its published notebook instead of the in-page editor. Deliberately owns no content: it never re-exports the bank and never touches Fly, it only syncs `Local_Deployed_Shared/` into the `deploy-colab` worktree and deploys. `deploy_delta_drills.sh` calls it as its last step (Step 7), non-fatally, because "one codebase, two deploys" is only true if both are published from the same tree.
...
10. Republish the Colab edition via `deploy_delta_drills_colab.sh` (non-fatal — the main release is already live; a failure here means the fork is stale, not that the release is bad).
...
## Recent Changes
- 2026-08-19: `deploy_delta_drills.sh` step 2 now also runs `scripts/compile_web_notebooks.py`, right after the question-bank export and the split-layout refresh. The in-app Notebooks tab reads compiled JSON under `Local_Deployed_Shared/lessons/notebooks/`, and those cells embed the exported bank — without this the rsync in step 4 would mirror a stale folder and the tab would teach the previous bank, rendering perfectly and showing no symptom. Plain `python3` is enough (no torch, ~2s); it is deterministic, so a no-op deploy rewrites the same bytes.
- 2026-08-07: `deploy_delta_drills.sh` gained **Step 7: republish the Colab edition**, calling `deploy_delta_drills_colab.sh` after the Fly deploy. Until now the main deploy published one of the two Vercel projects and stopped, so `delta-drills-colab.vercel.app` sat on whatever build it was last given by hand. That is not a cosmetic lag: the fork spent a week serving a frontend whose knowledge graph read its learner model from a `localStorage` key the backend no longer writes, so it drew 63 flat bubbles for an account with real practice behind it — indistinguishable from the backend outage fixed the day before, on a frontend nobody thought to redeploy. Non-fatal on purpose (the release is already live by then, and the colab script's own preflights can fail for reasons that say nothing about it), and `watch.py` now asserts the call is still there, since a step that only *warns* when it goes missing is the same failure again.
