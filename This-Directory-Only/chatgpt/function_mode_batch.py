...
DEFAULT_REQUESTS_PATH = BASE_DIR / "function_mode_requests.jsonl"
DEFAULT_OUTPUTS_PATH = BASE_DIR / "function_mode_overrides.jsonl"
DEFAULT_REJECTS_PATH = BASE_DIR / "function_mode_rejected.jsonl"
DEFAULT_DELETED_IDS_PATH = BASE_DIR / "function_mode_deleted_ids.json"
DEFAULT_QUALITY_CHECK_PROMPT_PATH = BASE_DIR / "function_mode_quality_check_system.txt"
DEFAULT_QUALITY_FIX_PROMPT_PATH = BASE_DIR / "function_mode_quality_fix_system.txt"
...
def validate_candidate(code_runner, source_item: dict, candidate: dict) -> tuple[bool, dict]:
    ...
    solution_code = synthesize_solution_code(starter_code, source_item.get("answer_code", ""))
...
async def main() -> None:
    ...
    api_key = load_api_key(str(BASE_DIR))
    if not api_key:
        raise RuntimeError("No OpenAI API key configured in chatgpt/.")

    client = AsyncOpenAI(api_key=api_key)
    model = os.environ.get("OPENAI_MODEL", get_configured_model(str(BASE_DIR))) or "gpt-4o-mini"
    lock = asyncio.Lock()
