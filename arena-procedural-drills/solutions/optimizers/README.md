# solutions/optimizers

## Purpose
- Reference-solution Colab notebooks for bank topic **`Optimizers`** — 9 of them, one per question that has authoring.
- Update rules and the running statistics that Adam, RMSprop and BatchNorm share.

## Owns
- `q<ID>-<subtopic-slug>.ipynb` for every `Optimizers` question with an authoring row.
- `q<ID>-*.problem.ipynb` — 6 answer-withheld variants, built by `build_problem_colabs.py`.
...
Subtopics covered:
- `SGD update rule` — 3
- `Parameter iteration and iterator exhaustion` — 3
- `EWMA running statistics (BatchNorm / Adam / RMSprop)` — 3
...
- **Topic-specific** — `ACTIVE`
  - EWMA running statistics appear both here and in the CNN BatchNorm material; they are the same idea filed under two topics.
