...
## Recent Changes
- 2026-09-25 (**difficulty debt CLEARED, both baselines SHRUNK**, 69→43 graph, 67→61 prose): the difficulty check walked six item lists, but the served ladder is two rungs — `faded`/`guided` (ranks 0/1) are retired from `kc_graph._STAGE_TO_RANKS`, and `applied`/`solo` are the `independent` list itself (compile_lessons fills them from the same ids). 23 of the 26 `difficulty|…` findings compared a subset with its own superset or a retired rung with a live one. `audit_graph_structure.SERVED_LADDER = ("independent", "integrated")` now. The other 3 were real, and each was a single-move drill filed as Integrated: re-cut by KIND, not by score — `einops.pooling` q917–919/929–931 (one reduce) → Solo, q336/q377 (pool AND move axes; the page already called them that) → Integrated beside q920; `einops.attention-einsum` q1561 (one projection) → Solo, q1558 (contract then scale) → Integrated; `cnn.linear-layer` q1200 (q1198 minus the bias) → Solo, q1196 (weight rows both ways + ReLU + residual) → Integrated. Integrated floors unchanged (3/2/3). Glossary "ray" → `math.ray-geometry` and "dot product" → `math.dot-products-norms`: the math pages define both first (ray-geometry is a prerequisite of ray-parametrisation), so the hover was skipping the definition and 6 prose hits were false positives. 🔴 Still open: 61 prose hits (≈23 in `lessons/mathematics/`, which quiz torch code and say "ray" before `math.ray-geometry`; the rest are the older python/tensor pages) and 43 `edge-missing`.
- 2026-09-25 (**both ratchet baselines re-recorded, with the reasons**): `graph_structure_baseline.json` +19 `difficulty|…` rung inversions (torch / einops / cnn), bisected to exactly the TypeSafe per-concept re-rate 3c7145e8 (0 new at its parent, 19 at it). That is the measured difficulty Seth shipped, not new content, so it is recorded rather than reverted; 2 healed entries dropped. `prose_prereq_baseline.json` +29 use-before-definition hits, all in the `lessons/mathematics/` pages (dad2ba36 + d18fff13 + later math edits; 29 already at fe08bc24). They went unseen because `mod watch` stops a file at its first failing check and the graph ratchet sat in front. Many are glossary false positives (`kp-ray-geometry` "says ray", `kp-dot-products-norms` "says dot product" — the math page itself teaches the term); the rest (tensor / shape / axis / mask / stack before torch.*) are real forward references. 🔴 Open content debt: reword per concept, one at a time. 2 healed entries dropped. `compile_arena_notebooks._cells` → public `notebook_cells` (test imported the private name). `test_validate_math` asserts the `MATH_TOPICS` message.
- 2026-09-22 (**authored lesson code has to fit the cell it is shown in**): new `lesson_width.py` — `MAX_CODE_COLS = 54`, `displayed_code()` (every fence a reader is shown: `python`, `python no-run`, `python worked`, visible half only — the `# Hidden checks` block is stripped before display and its `assert _delta_output == '...'` lines are a transcript that cannot be re-wrapped), `check_code_width()` and `check_page_width()`. `validate_lessons.py` calls it per page and **NOT under `strict_for`**: a legacy page that overflows its cell is as broken as a new one, and the whole 435-line backlog was cleared in the same pass rather than left visible and unfixed. `lesson_lib.py` gained `FENCE_RE`, the one fence split, so the two readers of the same markdown cannot drift about where a fence ends. 🔴 **54 is MEASURED, not chosen** — read out of a live browser on the lesson panel (682px measure × 0.95 zoom, less the 53px run gutter, 2px borders and 28px padding, over a 10.404px advance at 17.29px), which is the NARROWEST of the three surfaces a fence renders on, so one number rules all three. The paper derivation was wrong by 2px a side: inside a lesson `.nbv-md pre` beats `.nbv-src` and pays 14px. A TAB is refused outright rather than counted (its width on screen is not knowable from the source); no tab or double-width character appears in any fence today. `python starter` / `python solution` are deliberately out of scope — those render in the practice editor, which is `white-space: pre-wrap` and soft wraps instead of clipping. Seth, with a screenshot of `kp-linalg-basics` cut off mid-word at "compare with fl": *"the text needs to be fixed so that it doesn't extend past that part ... This also discourages things like this example, where it essentially tries to utilize printing the string alongside the code on the same line instead of breaking it up into multiple lines like it should."*
- 2026-09-21 (**math MC lane — software only, no content**): `validate_math.py` (+ `test_validate_math.py`, 37 unit tests on synthetic problems) — the gate for `kind: math` pages: problems-file shape, SymPy proof of every keyed choice against every distractor, rung placement vs frontmatter, atom-tag rows, placement-cap WARN; `validate_lessons.py` calls `check_math_kp` per math page and `check_all` for the tree. `lesson_lib.parse_kp` gains `kind`; `compile_lessons.py` tolerates a Worked example with no fence; `build_qmatrix.py` + `validate_lessons.py` add `Mathematics` to `EASY_TOPICS`; `colab_cells.mc_problem_cell` emits a math problem as a prose `dd-mc<id>` cell (choices, no key) and `generate_colab_notebooks.py` stops expecting a `dd-q<id>-check` anchor for it.
...
- 2026-09-01: `audit_prose_prereqs.py` added — 40 baseline findings (e.g. kp-dots-and-imports says "tensor" one lesson before torch.tensor-model defines it). Inline+fenced code stripped, mirroring jargon.js.
...
- 2026-08-29 (**both guards made standing, and a measurement bug that moved every ARENA number**): `guard_checks.py`, `audit_arena_grounding.py`, `arena_symbol_index.json`, `arena_grounding_baseline.json`, `audit_arena_frequency.py`, `audit_solution_prereqs.py`, `watch.py`, and the five lesson-folder `watch.py` files. Seth's ask: the prerequisite rule and the ARENA rule must both fire on the folder being edited, so nobody can add a drill and move on without them.
  ...
  - Both ratchets were proven in the failing direction (drop one baseline entry → the watcher fails; restore → green) and end to end: a synthetic drill using `torch.mvlgamma` on the first concept made `lessons/`, `lessons/tensors/` and `scripts/` all refuse at once.
