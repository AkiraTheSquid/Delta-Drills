...
2. Local_Deployed_Shared/lessons/pytorch/kp-{sequential,residual-block,block-group,resnet34,feature-extraction,training-loop,forward-hooks}.md  (lesson pages; frontmatter lists which drill ids each page owns under faded/guided/independent/integrated)
3. This-Directory-Only/questions_full.json — ONLY rows with id 1374..1470 (fields: question_text, starter_code, answer_code, test_cases, wrong_examples, difficulty_label, submission_mode, expected_artifact_type)

Do NOT edit anything. Do NOT run code that writes.

Grade every drill 1374..1470 and every lesson page listed. Be concrete: for each issue give code, severity, the exact quote, and a rewritten replacement sentence (not "be clearer"). Group repeated defects (e.g. the same cross-reference pattern) under one issue listing all ids. Then list the 10 worst items overall and what a LeetCode-quality version of the prompt would say.
