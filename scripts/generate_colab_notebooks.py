...
def build_index(records: list[dict], out_dir: Path) -> str:
    """The `question_id -> notebook` map the side panel needs, as a plain script.

    Emitted as JS rather than JSON because the panel loads it with a `<script>`
    tag: MV3's CSP forbids remote script but a bundled file is fine, and a
    script tag avoids `fetch` inside the panel, which would put a second HTTP
    client next to `api.js`.
...
def main() -> int:
    ...
    index_js = build_index(records, args.out)
    if not args.dry_run:
        args.index.parent.mkdir(parents=True, exist_ok=True)
        args.index.write_text(index_js)
    print(f"index · {len(records)} lessons · {total_problems} questions mapped · {args.index.name}")
