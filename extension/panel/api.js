...
const EMPTY_INDEX = {
  dir: "", lessons: [], questions: {}, subtopics: {}, kcs: {}, segments: {},
};
...
const notebooks = {
   ...
   * reason `slugKc` mirrors the generator and nothing else does: an anchor that
   * drifts by a character is one Colab ignores in silence, and the learner
   * lands at the top of a 650-cell notebook with nothing to say why.
   *
   * "" for a single-concept KP, and for any KP in an index generated before
   * segments existed. The caller falls back to `dd-kp-<slug>` — the anchor
   * every KP has had all along — so nothing regresses when this map is thin.
   */
  anchorForGate(gate) {
    if (!gate || !gate.exposure_key) return "";
    return (this.index.segments || {})[gate.exposure_key] || "";
  },
